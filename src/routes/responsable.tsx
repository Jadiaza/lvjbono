...
function ResponsibleDashboard() {
  ...
  const filtered = useMemo(
    () =>
      bonos.filter((b: any) => {
        ...
        const matchesSearch =
          !q ||
          padBonoNumber(b.serial).includes(q) ||
          b.raffle_bono_numbers?.some((n: any) => padBonoNumber(n.numero).includes(q));
  ...
  const raffleById = (raffleId: string) =>
    offerData?.batches?.find((batch: any) => batch.raffle.id === raffleId)?.raffle ?? null;
  ...
  function openBono(bono: any) {
    ...
    setPayment({
      amount_paid: String(bono.amount_paid || raffle?.valor_boleta || ""),
      payment_reference: bono.payment_reference ?? "",
    });
  }
  ...
  async function shareBatchCard() {
    ...
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      await navigator.share({
        title: selectedBatch.raffle.nombre,
        text: `${selectedBatch.raffle.nombre} · ${selectedBatch.name || selectedBatch.code}`,
        files: [file],
      });
      return;
    }
  ...
  async function buildIndividualBonoFile() {
    ...
    const dataUrl = await toPng(individualRef.current, {
      cacheBust: true,
      pixelRatio: 2,
    ...
    return {
      dataUrl,
      file: new File([blob], `bono-${padBonoNumber(selectedBono.serial)}.png`, {
        type: "image/png",
      }),
    };
  }
  ...
  async function sendSelectedBonoToWhatsApp() {
    if (!selectedBono?.buyer_phone) {
      toast.error("Este bono no tiene un WhatsApp registrado.");
      return;
    }
  ...
  const selectedNumbers = selectedBono
    ? (selectedBono.raffle_bono_numbers ?? [])
        .slice()
    ...
    <main className="mx-auto min-h-screen max-w-7xl space-y-6 p-3 sm:p-6">
      <header className="flex items-center justify-between gap-3 rounded-2xl border bg-card p-4">
        <div className="min-w-0">
          <h1 className="truncate font-display text-xl text-gold sm:text-2xl">
            {data.responsible.display_name}
          </h1>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {[
          ["Disponibles", s.counts.asignado ?? 0],
          ["Reservados", s.counts.reservado ?? 0],
          ["Vendidos", s.counts.vendido ?? 0],
          ["Pagados", s.counts.pagado ?? 0],
        ].map(([k, v]) => (
          <button
            key={String(k)}
            type="button"
            onClick={() => setFilter(String(k).toLowerCase() as any)}
      ...
      <section className="space-y-4 rounded-2xl border bg-card p-3 sm:p-5">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <h2 className="font-display text-lg sm:text-xl">Cartón para ofrecer</h2>
            <p className="text-sm text-muted-foreground">Toca un bono disponible para administrarlo.</p>
          </div>
          {selectedBatch && (
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={downloadBatchCard}>
                <Download className="mr-1 h-4 w-4" /> Descargar
              </Button>
              <Button size="sm" onClick={shareBatchCard}>
        ...
        <div className="flex flex-wrap gap-2">
          {(offerData?.batches ?? []).map((batch: any) => {
            ...
            return (
              ...
              >
                <Eye className="mr-1 h-4 w-4" /> {batch.name || batch.code} · {available}/
                {batch.bonos.length}
              </Button>
            );
          })}
        ...
        {selectedBatch ? (
          <BonoBatchCard
            ...
            onSelectBono={(bono: any) => openBono(bonos.find((b: any) => b.id === bono.id) ?? bono)}
          />
        ) : (
      ...
      <section className="space-y-3 rounded-2xl border bg-card p-3 sm:p-5">
          ...
          {filtered.map((b: any) => {
            ...
            return (
              <button
                ...
                key={b.id}
                onClick={() => openBono(b)}
                className="rounded-xl border bg-background p-4 text-left transition hover:border-gold/60 hover:shadow-sm"
              >
                <div className="flex items-center justify-between">
                  <strong>Bono {padBonoNumber(b.serial)}</strong>
                  <span className="rounded-full bg-secondary px-2 py-1 text-[11px] font-bold uppercase">
                    {b.status}
                  </span>
                </div>
                <div className="mt-3 text-2xl font-black text-red-700">{nums.join(" · ")}</div>
                {b.buyer_name && (
                  <p className="mt-2 text-sm">
                    <strong>Comprador:</strong> {b.buyer_name}
                  </p>
                )}
              </button>
            );
        ...
        {!filtered.length && (
          <p className="p-6 text-center text-sm text-muted-foreground">No hay bonos con este filtro.</p>
        )}
      </section>

      <ResponsibleBuyersTable
        bonos={bonos}
        raffleById={raffleById}
        onOpenBono={openBono}
        onRegisterPayment={openPaymentFor}
      />
      ...
      <Dialog open={Boolean(selectedBono)} onOpenChange={(open) => !open && setSelectedBono(null)}>
        <DialogContent className="max-h-[94vh] max-w-2xl overflow-y-auto p-4 sm:p-6">
          {selectedBono && (
            <>
              <DialogHeader>
                <DialogTitle>Bono {padBonoNumber(selectedBono.serial)}</DialogTitle>
              </DialogHeader>

              {selectedRaffle && (
                <div ref={individualRef}>
                  <BonoDualCard
                    statusDisplay="subtle"
                    bono={{
                      serial: selectedBono.serial,
                      verification_code: selectedBono.verification_code,
                      numbers: selectedNumbers,
                      status: selectedBono.status,
              ...
              {selectedBono.buyer_name && (
                <div className="rounded-xl bg-secondary p-3 text-sm">
                  <p>
                    <strong>Comprador:</strong> {selectedBono.buyer_name}
                  </p>
                  <p>
                    <strong>Teléfono:</strong> {selectedBono.buyer_phone || "—"}
                  </p>
                  {selectedBono.buyer_city && (
                    <p>
                      <strong>Ciudad:</strong> {selectedBono.buyer_city}
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {AVAILABLE.has(selectedBono.status) && (
                  <Button onClick={() => setReserveOpen(true)}>Reservar este bono</Button>
                )}
                {selectedBono.status === "reservado" && (
                  <>
                    <Button onClick={registerSale} disabled={busy}>
                      Registrar venta
                    </Button>
                    <Button variant="outline" onClick={cancelSelectedReservation} disabled={busy}>
                      Cancelar reserva
                    </Button>
                  </>
                )}
                {selectedBono.status === "vendido" && (
                  <>
                    <Button onClick={() => setPayOpen(true)}>Registrar pago</Button>
                    <Button variant="outline" onClick={revertSelectedSale} disabled={busy}>
                      <Undo2 className="mr-1 h-4 w-4" /> Revertir venta
                    </Button>
                  </>
                )}
                {selectedBono.buyer_phone && (
                  <Button className="sm:col-span-2" onClick={sendSelectedBonoToWhatsApp}>
                    <Share2 className="mr-1 h-4 w-4" /> Enviar bono por WhatsApp
                  </Button>
                )}
              </div>
      ...
      <Dialog open={reserveOpen} onOpenChange={setReserveOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>¿A quién se reserva?</DialogTitle>
          </DialogHeader>
          <form onSubmit={submitReservation} className="space-y-3">
            <Field label="Nombre del comprador *">
              <Input
                ...
                onChange={(e) => setCustomer({ ...customer, buyer_name: e.target.value })}
              />
            </Field>
            <Field label="Teléfono / WhatsApp *">
              <Input
                ...
                minLength={7}
                inputMode="tel"
                value={customer.buyer_phone}
                onChange={(e) => setCustomer({ ...customer, buyer_phone: e.target.value })}
              />
            </Field>
            <Field label="Ciudad">
              <Input
                value={customer.buyer_city}
                onChange={(e) => setCustomer({ ...customer, buyer_city: e.target.value })}
              />
            </Field>
            <Field label="Nota">
              <Input
                value={customer.buyer_notes}
                onChange={(e) => setCustomer({ ...customer, buyer_notes: e.target.value })}
              />
            </Field>
          ...
          <form onSubmit={submitPayment} className="space-y-3">
            <Field label="Valor recibido">
              <Input
                ...
                onChange={(e) =>
                  setPayment({ ...payment, amount_paid: e.target.value.replace(/\D/g, "") })
                }
              />
            </Field>
