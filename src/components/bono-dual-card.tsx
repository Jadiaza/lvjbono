...
function NumberPanel({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="overflow-hidden rounded-[clamp(.35rem,.8vw,.65rem)] border border-[#5b331b]/50 bg-white/94 shadow-md">
      <div className="bg-[#4b2814] px-1 py-[1.15%] text-center text-[clamp(.24rem,.46vw,.39rem)] font-black uppercase tracking-[.08em] text-white">{label}</div>
      <div className="grid grid-cols-3 gap-[2.8%] px-[6.4%] py-[2.2%]">
        {padBonoNumber(value).split("").map((digit, index) => (
          <div key={index} className="flex aspect-[1.18/1] items-center justify-center border border-[#5b331b]/45 bg-white font-black leading-none shadow-inner" style={{ color, fontSize: "clamp(.66rem,1.82vw,1.45rem)" }}>{digit}</div>
        ))}
      </div>
    </div>
  );
...
export const BonoDualCard = forwardRef<HTMLDivElement, { bono: DualBonoCardData; verifyUrl?: string; statusDisplay?: StatusDisplay }>(
  ({ bono, verifyUrl, statusDisplay = "review" }, ref) => {
    ...
    const status = bono.status?.toLowerCase() || "";
    ...
    return (
      <div ref={ref} className="relative aspect-[3/2] w-full overflow-hidden rounded-[1.3rem] border bg-[#f5e8ce] text-[#30190c] shadow-xl" style={{ borderColor: accent }}>
        <div className="absolute inset-0" style={{ background: "radial-gradient(circle at 36% 8%, #fffdf7 0%, #f8efd9 42%, #e5c895 100%)" }} />
          ...
          <section className="relative min-w-0 overflow-hidden border-r border-dashed border-[#6c4a2a]/45 px-[3%] pb-[7%] pt-[1.6%]">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1 text-center">
                <div className="leading-[.8]" style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "clamp(1.65rem,5.35vw,4.18rem)", fontWeight: 700 }}>{raffle.bono_title || "Bono"}</div>
                <div className="mx-auto mt-[.4%] max-w-[98%] leading-[.88]" style={{ fontFamily: "'Brush Script MT', 'Segoe Script', 'Apple Chancery', cursive", fontSize: "clamp(1.08rem,3.55vw,2.82rem)", fontWeight: 500 }}>{raffle.bono_subtitle || raffle.nombre}</div>
              </div>
            </div>

            <div className="mt-[.9%] grid h-[66%] grid-cols-[60%_40%] gap-[1.4%]">
              <div className="flex items-end justify-center overflow-hidden pb-[1%]">
                {prizeImage ? <img src={prizeImage} alt={raffle.bono_prize_name || "Premio"} className="h-[99%] w-full object-contain object-bottom" /> : <div className="text-xs text-[#6b5a48]">Imagen del premio</div>}
              </div>
              <div className="flex min-w-0 flex-col justify-start pr-[2.4%] pt-[8%] text-left">
                <div className="leading-[1.06]" style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "clamp(.54rem,1.55vw,1.1rem)", fontWeight: 700 }}>{raffle.bono_prize_description || `Se rifa esta hermosa ${raffle.bono_prize_name?.toLowerCase() || "cajonera archivador"}.`}</div>
                <OrnamentDivider />
                {raffle.bono_prize_dimensions && <div className="text-center leading-tight text-[#3e2415]" style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "clamp(.4rem,1.02vw,.77rem)", fontWeight: 700 }}><div>Medidas</div><div>{raffle.bono_prize_dimensions}</div></div>}
                {date && (
            ...
            <div className="absolute bottom-0 left-0 right-0 flex h-[7%] items-center justify-center bg-[#4d2812] px-4 text-center text-[clamp(.46rem,1vw,.78rem)] italic text-white" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>{raffle.bono_footer_text || "¡Gracias por apoyar nuestra misión!"}</div>
          </section>

          <section className="relative min-w-0 overflow-hidden bg-[#d9a94f]">
            {sideImage ? <img src={sideImage} alt="San Miguel Arcángel" className="absolute inset-0 h-full w-full object-cover object-top" /> : <div className="absolute inset-0 bg-gradient-to-b from-[#f3d79c] to-[#c98e31]" />}
            ...
            <div className="absolute inset-x-[6.2%] bottom-[3.2%] z-10 space-y-[1.7%]">
              ...
              <div className="rounded-[clamp(.35rem,.8vw,.65rem)] border border-[#5b331b]/45 bg-white/95 px-[6%] py-[2.6%] text-center shadow-md"><span className="mr-3 text-[clamp(.68rem,1.55vw,1.2rem)] font-black" style={{ color: numberColor }}>Nº</span><span className="text-[clamp(1.05rem,2.45vw,1.95rem)] font-black tracking-[.03em]">{padBonoNumber(bono.serial)}</span></div>
              {verifyUrl && raffle.bono_show_qr !== false && <div className="truncate rounded bg-white/75 px-1 text-center text-[clamp(.2rem,.38vw,.3rem)] font-semibold text-[#3f2b1b]">Verificación: {verifyUrl}</div>}
            </div>
          </section>
        </div>
